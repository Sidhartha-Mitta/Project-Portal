  ...
  React.useEffect(() => {
    if (project?.team?.members) {
      const initialRatings = project.team.members
        .filter(
          (member) => member.status === "active" && member.role !== "client"
        )
        .map((member) => ({
          member: member.user._id,
          memberName: member.user?.name || "Unknown Student",
          rating: 5,
          feedback: "",
        }));
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 bg-opacity-50 flex items-center justify-center z-50 p-4 mt-30"
          onClick={onClose}
        >
          <motion.div
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      {memberRatings.map((memberRating, index) => {
                        return (
                          <div
                            key={memberRating.member}
                            className="border border-gray-200 rounded-lg p-4"
                          >
                            <h5 className="font-medium text-gray-900 mb-3">
                              {memberRating.memberName ||
                                `Student ${index + 1}`}
                            </h5>
                            ...
                            <div className="mb-3">
                              ...
                              <div className="flex items-center space-x-1">
                                {renderStars(memberRating.rating, (newRating) =>
                                  handleMemberRatingChange(
                                    memberRating.member,
                                    "rating",
                                    newRating
                                  )
